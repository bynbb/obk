...
| --- | --- |
| ChatGPT / Codex | Tool |
| Gritlabs Project Scaffold | Component of Gritlabs - a Prompt management and traceability system |
...
- 2.4 Aenean eu leo quam. Pellentesque ornare sem lacinia quam  
- 2.5 Vestibulum id ligula porta felis euismod semper  
...
| N-1 | Pay Chinese restaurant | Completed |
| N-2 | Tip driver | Incomplete |
| N-3 | Update `<gsl-purpose>` in prompt 20250727T004356-0400 | Incomplete |
...
## 7. Grit Structured Language (GSL) Specification
...
S-3.4 These restrictions ensure the integrity of the specification, audit trails, and test cases, while allowing agents to manage workflow steps and update next-step progress.
...
```xml
<gsl-tdd>
<gsl-test id="T1">Running `obk generate prompt` should generate a new prompt in the proper deterministic date folder, for example `prompts/07/27/20250727T004356-0400.md`.</gsl-test>
<gsl-test id="T2">Running `obk generate prompt-id` should generate an ID following the format YYYYMMDDTHHMMSS±HHMM</gsl-test>
</gsl-tdd>
```

S-5.4 Manual tests MAY be used for auditing, traceability, and qualitative assessment.
