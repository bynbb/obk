...
- T2: CLI output reflects overridden greeter implementation
    ```python
    container.greeter.override(MockGreetingService())
...
- T3: Divider override returns custom value
    ```python
    container.divider.override(MockDivider())
    ObkCLI(container=container).run(["divide", "1", "2"])
    ```
